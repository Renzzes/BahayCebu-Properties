...
const Navbar: React.FC = () => {
  ...
  const onLoginSubmit = async (values: z.infer<typeof loginSchema>) => {
    try {
      ...
      // Optionally, store user info in state or localStorage here
    } catch (err: unknown) {
      ...
      <div className="container-custom flex items-center justify-between py-4">
        ...
        <div className="flex items-center gap-2">
          <div className="hidden lg:flex items-center gap-2">
            <Button 
              variant="outline" 
              className="border-bahayCebu-green text-bahayCebu-green hover:bg-bahayCebu-green hover:text-white"
          ...
          <button 
            onClick={toggleMenu}
            className="lg:hidden text-bahayCebu-darkGray p-2 focus:outline-none"
          >
            {isMenuOpen ? (
              <X className="h-6 w-6" />
            ) : (
        ...
        <div className="fixed inset-0 top-16 bg-white z-40 p-5 lg:hidden">
          <nav className="flex flex-col space-y-4 text-lg">
            <Link 
              to="/"
              onClick={toggleMenu} 
              className="text-bahayCebu-darkGray hover:text-bahayCebu-green py-2 font-medium border-b border-gray-100 transition-colors relative"
            >
              Home
            </Link>
            <button 
              onClick={handlePropertiesClick}
              className="text-left text-bahayCebu-darkGray hover:text-bahayCebu-green py-2 font-medium border-b border-gray-100 transition-colors relative"
            >
              Properties
            </button>
            <Link 
              ...
              {authMode === 'login' ? (
                <p>Don't have an account?{' '}
                  <button 
                    ...
                    className="text-bahayCebu-green hover:underline font-medium"
                  >
                    Sign up
                  </button>
                </p>
